...
describe('loadProfile', () => {
  ...
  it('resolves org and project selectors', async () => {
    ...
    const r = await loadProfile({ org: 'acme', project: 'CORE', configPath });
    expect(r.org.name).toBe('acme');
    expect(r.project.key).toBe('CORE');
    expect(r.config.projectKey).toBe('CORE');
    expect(r.config.baseUrl).toBe('https://acme.atlassian.net');
    expect(r.project.outputDir).toMatch(/jira\/acme\/core$/);
